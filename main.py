...
NEIGHBORHOOD_DIFFERENCE_THRES = 0.5
...
def get_border_pixels(width, height, grayscale, neighborhood):
    ...
    for x in range(width):
        for y in range(height):
            pts = points_within_distance(x, y, width, height, neighborhood, lambda i, j: grayscale[i][j])
...
def main(in_file, out_file, neighborhood):
    ...
    grayscale = [[sum(distance(pix[x, y], nbr)
        for nbr in points_within_distance(x, y, width, height, neighborhood, lambda i, j: pix[i, j]))
        for y in range(height)]
        for x in range(width)]
